	...
	<view class="Box" style="">
		...
		<view class="topBox anmt" :style="{color:fontColor,backgroundColor:menuBg,top:show?'0':'-150px'}">
			...
			<view style="height: 100upx;line-height: 100upx;text-overflow: ellipsis;" class="tMain">
				...
				<text v-if="getToken" class="cuIcon-favorfill" :class="[isCollected?'text-orange':'']" @tap="comicCollectAction"></text>
				<text v-if="getToken" class="cuIcon-appreciate" :class="[isLiked?'text-orange':'']" @tap="comicLikeAction"></text>
			</view>
		</view>
		<!-- 带返回键的导航栏结束 -->
		<!-- 菜单开始 -->
		...
		<!-- 顶部结束 -->
		<!-- 小说正文开始 -->
		<view class="sview" style="color: rgb(0, 0, 0); font-size: 55px; line-height: 27px;">
			<!-- <image v-for="(item,index) in content_text" :key="index" :src="item.pic" mode="scaleToFill" style="width: 100vw;"></image> -->
			<loayImg v-for="(item,index) in content_text" :key="index" style="width: 100vw;" :imgUrl="item.pic" :fill="'scaleToFill'"></loayImg>
		</view>
		<!-- 小说正文结束 -->
		<!-- start抽屉 -->
		...
		<scroll-view scroll-y class="DrawerWindow" :class="modalName=='viewModal'?'show':''">
			<view class="cu-list menu card-menu margin-top-xl margin-bottom-xl shadow-lg">
				<view class="cu-item arrow" v-for="(item,index) in getCapterComicList" :key="index">
					<view class="content">
		...
		<scroll-view scroll-y class="cu-modal drawer-modal justify-start" :class="modalName=='DrawerModalL'?'show':''" @tap="hideModal">
			...
			<view class="cu-dialog basis-lg" @tap.stop="" style="width: 500rpx;text-align: none;">
				<view class="cu-list menu text-left">
					<view class="cu-item arrow" v-for="(item,index) in getCapterComicList" :key="index" @tap="tocapter(item.id)">
						<view class="content">
...
export default{
	...
	computed:{
		...mapGetters({
			getCapterComicList: "getCapterComicList",
			getcomicStatus: "getcomicStatus",
			getToken:"getToken"
		}),
	},
	onUnload() {
	...
	created() {
		this.isCollected = this.getcomicStatus.isCollected
		this.isLiked = this.getcomicStatus.isLiked
		var this_ = this;
		//获取状态栏高度给顶部占位节点
	...
	onShow() {
		// console.log(this.getCapterComicList)
		//页面显示的时候将通知栏隐藏掉
		plus.navigator.setFullscreen(true);
	},
	onHide() {
	...
	onReachBottom() {
		// console.log(this.num, 7777)
		if (this.isLoadMore) {
			this.num += 1
			// this.jiNo = this.getCapterBookList[this.num].jiNo,
			this.chapterId = this.getCapterComicList[this.num].id
			this.episodesContentAction()
	...
	methods:{
		...
		back(){
			uni.navigateBack({});
			// uni.navigateTo({
			// 	url: "/pages/two/comic/comicDetail/comicDetail?comicId=" + this.comicId
			// })
		},
		dianjile(){
			this.show=!this.show;
		...
		comicCollectAction() {
			...
			API.closedComic(data).then(
			        res => {
						...
						this.$store.dispatch("setStatusComic", setStatusComic).then(()=>{
		...
		episodesContentAction() {
			...
			let data = { chapterId:this.chapterId }
			this.content_text = []
			this.modalName = null
			API.episodesContent(data).then(
			        res => {
						...
						let getCapterComicList = JSON.parse(JSON.stringify(this.getCapterComicList))
						getCapterComicList.forEach((item, index)=>{
							if(item.id == this.chapterId) {
								this.section_title = item.title
								this.num = index
								if (index>=getCapterComicList.length-1) {
