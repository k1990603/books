<template>  
    <view class="my">
		<view class="myTitle">
			...
			<view class="container flex  p-xs margin-bottom-sm mb-sm">
				...
				<view class="flex-twice text-white margin-xs radius" style="fontSize:28rpx">
					...
					<view class="text">
					  <!-- <span style="display: block;" class="title">密码：admin123456</span> -->
					 <span v-if="userInfo.isVip" style="display: block;">VIP到期时间：{{ userInfo.vipEndTime }}</span>
					 </view>
				</view>
				<view class="flex-sub text-white margin-xs radius">
		...
		<view class="bg-white grid col-2 padding bookWrap">
			...
			<view class="text-center solids-right">
				<text class="text-gradual-orange text-xxl">{{ userInfo.bookCurrency }}</text>
			</view>
			<view class="text-center text-lg">
				<text class="text-gradual-orange text-xxl">{{ userInfo.commission || 0 }}</text>
			</view>
			<!-- <view></view> -->
		</view>
		<!-- 书币 -->
		<view class="cu-list menu">
			<view class="cu-item">
				...
				<div class="action">
					<button class="cu-btn round lines-orange text-sm" @tap="toNavigate" data-url="/pages/user/charge/charge">
						立即充值
					</button>
				</div>
			</view>
			...
			signAction() {
				API.signIn().then(
					        res => {
								...
								uni.showToast({ icon: 'none', title: '签到成功', duration: 1500 });
					        },
					        err => {}
					      );
